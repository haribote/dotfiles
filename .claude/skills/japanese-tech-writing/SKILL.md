...
description: "日本語の技術文書の文章規範。整形（一文一行、引用ブロック、脚注）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除、見出しの付け方を定める。記事・README・解説・ブログ・PR や issue の説明・設計メモなど、日本語の技術文書を書く・推敲する・リライトするときは必ず使う。日本語の説明文や文章を整える場面でも発動する。"
user-invocable: true
---

# 日本語技術文書の文章規範

日本語で技術文書（記事、README、解説、ブログ、PR や issue の説明、設計メモなど）を書く・推敲するときは、以下の規範に従う。
...
- 見出しや補足・囲みの見出しに、区切り線（罫線 `─` U+2500 やダッシュ類）で「種別──主題」「主題──概念」のように二要素を詰め込まない。見出しは単一の自然な句にする（要素を一つに絞るか、助詞・読点でつなぐ）。補足や囲みの見出しも「基礎」「補足」のような種別名だけで済ませず、「同値関係としての分類」「ループ不変条件と帰納法」のように内容を特定する。
...
- 例への弁明（作為的に見える、への先回りなど）は、場面の山場の直後に挟んで流れを切らない。次の見出しの冒頭でまとめて処理する。
...
- 「後述する」「別の箇所で扱う」のような前方参照は、論証が一段落した位置（段落末・節末）に置く。論証の途中に挟んで流れを切らない。
...
- 推量・可能性・読者の疑念・反実仮想として書かれている文を、機械的に断定へ変えない。
  ...
  事実未確認の可能性、当事者の認識、ログからの推定、読者が抱きそうな疑念、反実仮想を表す場合は、その不確実性を保つ。
...
- 文書全体をまたいで、同じ概念の扱いを一致させる。ある箇所で「人間が決める」と分類したものを、別の箇所で「チームで合意する」と書かない。分類・定義・用語の地位は文書全体で揃える。
...
- 「後述する」と前方に逃がした論点は、本当にそこで回収されることを確認する。回収しない伏線を張らない。
...
- 文書や見出しの導入では、これから例で扱う内容に関係しない過剰な詳細を詰め込まない。
- 具体例のまとまりの中でも、**その例の問い・帰結に関係しない**過剰な詳細だけを omit する。議論に必要な具体は残す。省略の典型は、装飾的な精度（時刻、HTTP ステータス、カバレッジ率など）や、後で参照しない固有名。
...
- 文書内で定式化・術語（契約、不変条件など）を導入したら、以後はその語で通す。「文脈」「ツール」「AI」のような曖昧語に後退しない（定式化する前の導入語として「文脈」などを使うのはよい）。
...
- **予告と総括**：「重要なのは〜である」「本稿では〜を扱う／探求する」「ここでは〜について見ていく」「まとめると」「要するに」（直前の言い換えだけのとき）、「〜に他ならない」
...
- **弱い緩和と称賛**：「〜と言えるだろう」「〜かもしれない」（根拠なく主張を弱める場合だけ。推量・仮定・読者の疑念・当事者の認識なら残す）、「非常に」「極めて」「大いに」（中身のない強調）
...
- 「筆者もそれを否定しない」のような、書き手の立場の弁明や断りを書かない。事実の記述（「〜に書かせる場合が多い」）だけを置く。
...
- 技術用語・コード・変数名・コマンド・ログ出力は英語のまま書く。無理に日本語へ訳さない（例: race condition を「競合状態」と訳さず、そのまま使ってよい）。
- 事実と推測を分けて書く。確認していないことを、確認したかのように断定しない（「読者への誠実さ」を、書き手自身の断定にも適用する）。
