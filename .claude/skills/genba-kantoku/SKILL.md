...
- サブタスクが3件以上あり、それぞれ独立したファイル・領域を触る（互いにブロックしない、あるいは依存関係が明確で並行実行グループ「Wave」に分けられる）
...
- サブタスク同士の依存関係を洗い出し、並行可能なグループ（Wave）に分ける。**1つのファイルを複数の helper-agent が同時に触らないよう、ファイル単位で担当を切り分ける**。
