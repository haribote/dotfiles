...
```bash
herdr worktree create --workspace <ID> --branch <name> --base main --path .claude/worktrees/<worktree-name> --label "<label>" --no-focus
```

- `--workspace <ID>` と `--cwd <PATH>` は排他。両方渡すと `Exit code 2` になる。
- `--path` は必ず指定する。省略すると herdr のデフォルト位置に作られ、Claude Code 自身が worktree を作るときのディレクトリ規則（リポジトリ直下の `.claude/worktrees/<worktree-name>`）とズレる。
