...
- 依存コマンド: `starship`, `fzf`, `fd`, `eza`, `bat`, `delta`, `gh`, `ghq`, `jq`, `nodenv`, `rg`, `nvim`, `mmdc`（mermaid-cli）, `magick`（imagemagick）。未インストールでも該当機能が無効になるだけで致命的には壊れない作りにしてある。全て `.Brewfile` に含まれる。
