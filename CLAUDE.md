...
- **Claude Code** (`.claude/`): グローバル個人設定 (`CLAUDE.md`)・エディタ設定 (`settings.json`: permissions allowlist・SessionStart フック〔herdr agent 状態通知〕・有効プラグイン宣言 `enabledPlugins`〔公式 4 つ〕)・サブエージェント (`agents/`: `tdd-expert`)・skills (`skills/`: `commit`, `japanese-tech-writing`, `pr-lifecycle`, `typescript-conventions` の実体と、クロスエージェント共用 skill `herdr` / `find-skills` への symlink) を追跡する。プラグインの実体（`~/.claude/plugins/` 配下の `marketplaces/`・`cache/`・`installed_plugins.json` 等）は `.gitignore` で除外し、settings.json で有効化の宣言だけを追跡する（実体は各マシンで `claude plugin install` または起動時に公式マーケットプレイスから取得する。手順は README 参照）。会話ログ・キャッシュ・セッション・`settings.local.json` 等のマシン固有/機密データは `.gitignore` の whitelist で除外している（`.claude/*` を無視し、`CLAUDE.md` / `settings.json` を再 include。`agents/` と `skills/` はディレクトリ全体ではなく、宣言済みの個別エントリ＝ `tdd-expert` と上記4 skill、および `herdr` / `find-skills` の symlink だけを per-entry で再 include する。これによりプラグインや手動インストールで入ったサードパーティ製の skill / agent は自動的に追跡対象外になる）。`herdr` / `find-skills` はクロスエージェント共用 skill で、実体は `.agents/skills/` に置き、`.claude/skills/` からはリポジトリ内相対の symlink（`../../.agents/skills/...`）で参照する。相対にすることでマシン固有の絶対パスや repo の配置深度を埋め込まず、クローンだけで解決する。`~/.agents` も本 repo の `.agents` への symlink にすると、find-skills のインストーラや他エージェントが同じ実体を共有する。インストーラが生成する `.agents/.skill-lock.json`（マシン固有）は `.gitignore` で除外する。
