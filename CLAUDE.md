...
- **ghostty** (`.config/ghostty/config`): フォント `UDEV Gothic NF`、テーマ `Material Design Colors`。pane/tab/zoom/copy 操作は基本 herdr が prefix（`Ctrl+a`）キーで自前処理するため ghostty 側のキー橋渡しは持たない。例外として `Cmd+T`/`Cmd+Shift+T`/`Cmd+Option+T`/`Cmd+W` の4つだけ、`text:` アクションで一意な `ctrl+alt+<文字>` の生バイト列を herdr へ送出するブリッジを持つ（`.config/herdr/config.toml` の直接バインドと対で機能する）。`Cmd+Shift+N` は ghostty ネイティブの新規ウィンドウ。
- **herdr** (`.config/herdr/config.toml`): ワークスペース/タブ/ペイン・エージェント管理ツールの設定。prefix は `[keys] prefix = "ctrl+a"` で上書きし、pane/tab/zoom 操作や copy mode（`prefix+[`→`v` 選択→`y` でクリップボードへ）は herdr 既定キーで運用する（split は `new_cwd = "follow"` 既定で親の cwd を継承）。加えて `split_horizontal`/`split_vertical`/`new_tab`/`new_workspace`/cycle 系アクションに `ctrl+alt+<文字>` の直接バインドを配列で追加し、ghostty の Cmd ショートカットから届く chord を受ける。`Cmd+W`（`ctrl+alt+w`）は `[[keys.command]]` でスクリプト `.config/herdr/scripts/herdr-cmd-w.sh` を呼び、pane 複数→pane close／tab 複数→tab close／それ以外→workspace close を y/N 確認の上で分岐する（ghostty の close 確認ダイアログはこの bridge で `Cmd+W` に対しては発火しなくなるため、herdr 側の対話ペインで代替）。未解決事項2点は issue #28 参照: ① `Cmd+N` の「herdr 非稼働時は新規ウィンドウにフォールバック」は ghostty がプロセス条件分岐できないため実装していない（`Cmd+N`/`Cmd+Shift+N` は素の `new_window` のまま）。② 最後の1ワークスペースを閉じた際に herdr が空ワークスペースへ自己回復するのか ghostty ウィンドウごと閉じるのかは未検証。テーマ・トースト通知・UI 表示・キー設定のみ追跡し、セッション状態（`session.json`）とログ（`herdr-server.log`／`herdr-client.log`）は `.gitignore` で除外する。Claude Code の SessionStart フック（`~/.claude/hooks/herdr-agent-state.sh`）と連携してエージェント状態を herdr へ通知する（フック実体は `~/.claude/hooks/` 配下でマシンローカル・追跡外）。
